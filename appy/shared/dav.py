# ------------------------------------------------------------------------------
import os, re, http.client, stat, urllib.parse, time, socket, xml.sax
...
class Resource:
    ...
    def updateHeaders(self, headers):
        ...
        headers['Authorization'] = "Basic %s" % base64.b64encode(credentials.encode()).decode()
    ...
    def send(self, method, uri, body=None, headers={}, bodyType=None):
        ...
        if self.measure: self.serverTime += duration
    ...
    def soap(self, data, uri=None, headers={}, namespace=None, soapAction=None):
        ...
        headers['Content-Length'] = str(len(body))
        if soapAction: headers['SOAPAction'] = '"%s"' % soapAction
        return self.send('POST', uri, headers=headers, body=body)
